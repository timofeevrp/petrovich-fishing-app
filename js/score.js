...
const FACTOR_LABELS = {
  pressure: { up: "Давление стабильно/падает — хороший знак", down: "Давление резко растёт — клёв может ухудшиться" },
...
export function scoreInterpretation(score) {
  if (score <= 15) return { emoji: "🔴", label: "Очень низкий шанс", tier: 0, text: "Условия сегодня неблагоприятны, лучше выбрать другой день" };
  if (score <= 35) return { emoji: "🟠", label: "Слабый клёв", tier: 1, text: "Клёв возможен, но условия не в вашу пользу" };
  if (score <= 55) return { emoji: "🟡", label: "Средний потенциал", tier: 2, text: "Шансы есть, стоит попробовать в правильное время суток" };
  if (score <= 75) return { emoji: "🟢", label: "Хороший шанс", tier: 3, text: "Условия располагают к клёву, стоит ехать" };
  if (score <= 90) return { emoji: "🟢", label: "Отличные условия", tier: 4, text: "Один из лучших дней за последнее время — не упустите" };
  return { emoji: "🟩", label: "Пик активности", tier: 4, text: "Редкое сочетание факторов, максимальный потенциал" };
}
